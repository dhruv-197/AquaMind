...
export async function fetchStressStatus(): Promise<StressStatus> {
  return apiGet<StressStatus>('/api/v1/predictions/stress/status');
}

export async function predictStress(body: {
  region_id?: string;
  horizon_days?: number;
  reservoir_id?: string;
  ...
  return apiPost('/api/v1/predictions/stress/predict', {
...
export async function simulateStress(body: {
  region_id?: string;
  horizon_days?: number;
  reservoir_id?: string;
  scenario?: StressScenario;
  ...
  return apiPost('/api/v1/predictions/stress/simulate', {
    region_id: body.region_id,
    horizon_days: body.horizon_days ?? 30,
    reservoir_id: body.reservoir_id,
    scenario: body.scenario || {},
